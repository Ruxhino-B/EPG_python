<?php
	...
	$mychannels = simplexml_load_file('allchannels_recover.xml');
	$channelID = simplexml_load_file('/root/epg/channelID.xml');
	
	$myfile = fopen("Program_recover.dat", "w") or die("Unable to open file!");
	...
	fclose($myfile);
	$myfile = fopen("Schedule_recover.dat", "w") or die("Unable to open file!");
        for($i=0; $i<count($mychannels->children()); $i++)
		for($k=0; $k<count($channelID->children()); $k++){
                        if( strcmp($mychannels->channel[$i]['name'], $channelID->channel[$k]['name'])== 0){
